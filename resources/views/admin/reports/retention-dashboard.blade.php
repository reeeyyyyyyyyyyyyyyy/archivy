                            ...
                            <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Arsip</th>
                                            ...
                                            <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sisa Hari</th>
                                        </tr>
                                    </thead>
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @foreach($approachingInactive as $archive)
                                            @php
                                                $daysRemaining = today()->diffInDays($archive->transition_active_due, false);
                                                $urgencyClass = $daysRemaining <= 7 ? 'bg-red-100 border-red-200' : ($daysRemaining <= 30 ? 'bg-orange-100 border-orange-200' : 'bg-yellow-100 border-yellow-200');
                                            @endphp
                            ...
                            <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Arsip</th>
                                            <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nasib Akhir</th>
                                            <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Jatuh Tempo</th>
                                            <th class="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sisa Hari</th>
                                        </tr>
                                    </thead>
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @foreach($approachingFinal as $archive)
                                            @php
                                                $daysRemaining = today()->diffInDays($archive->transition_inactive_due, false);
                                                $urgencyClass = $daysRemaining <= 7 ? 'bg-red-100 border-red-200' : ($daysRemaining <= 30 ? 'bg-orange-100 border-orange-200' : 'bg-yellow-100 border-yellow-200');
                                            @endphp
                                            <tr class="{{ $urgencyClass }}">
                                                <td class="px-3 py-4">
                                                ...
                                                <td class="px-3 py-4 text-sm text-gray-900">{{ $archive->transition_inactive_due->format('d/m/Y') }}</td>
                                                <td class="px-3 py-4">
                                                    <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $daysRemaining <= 7 ? 'bg-red-200 text-red-800' : ($daysRemaining <= 30 ? 'bg-orange-200 text-orange-800' : 'bg-yellow-200 text-yellow-800') }}">
                                                        {{ $daysRemaining }} hari
                                                    </span>
                ...
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @foreach($archivesByCategory as $category)
                            @php
                                $percentage = $stats['total_archives'] > 0 ? ($category->count / $stats['total_archives']) * 100 : 0;
                            @endphp
                            <div class="flex items-center">
                                <div class="flex-1">
                                    <div class="flex justify-between text-sm">
                                        <span class="font-medium text-gray-900">{{ $category->name }}</span>
                                        <span class="text-gray-500">{{ $category->count }} arsip ({{ number_format($percentage, 1) }}%)</span>
                                    </div>
