...
class CreateTestArchiveCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $activeDue = $startDate->copy()->addYears($classification->retention_aktif);
        $inactiveDue = $activeDue->copy()->addYears($classification->retention_inaktif);

        $archive = Archive::create([
            'category_id' => $category->id,
            'classification_id' => $classification->id,
            'index_number' => "TEST-{$year}-" . str_pad(rand(1, 999), 3, '0', STR_PAD_LEFT),
            'description' => "Test archive from {$year} for status transition testing",
